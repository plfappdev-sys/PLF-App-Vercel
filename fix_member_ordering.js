...
async function fixMemberOrdering() {
    console.log('🔧 Fixing member ordering issue...');
    ...
    try {
        ...
        // Show current ordering issue
        console.log('📋 Current member order (showing the problem):');
        members.slice(0, 15).forEach(member => {
        ...
        // Verify the updates with proper numeric sorting
        console.log('🔍 Verifying updates with correct ordering...');
        ...
        if (verifyError) {
            ...
            const numericallySorted = updatedMembers.sort((a, b) => {
                return parseInt(a.member_number) - parseInt(b.member_number);
            });
            
            console.log('📋 First 15 members with correct ordering:');
            numericallySorted.slice(0, 15).forEach(member => {
                console.log(`  ${member.member_number}. ${member.name}`);
            });
        }
