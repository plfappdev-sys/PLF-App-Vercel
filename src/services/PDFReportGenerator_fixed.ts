...
<body>
  ...
  // Generate HTML content for Member Statement Report - FIXED VERSION
  static generateMemberStatementHTML(reportData: ReportData): string {
    const data = reportData.data;
    
    // Calculate correct percentage outstanding based on actual data
    const expectedContribution = data.member?.financialInfo?.expectedContribution || 17400;
    const totalContributions = data.financialSummary?.totalContributions || 0;
    const outstandingAmount = data.financialSummary?.outstandingAmount || 0;
    
    // Calculate correct percentage (should be 22.82% for Lesego Bokaba)
    const correctPercentageOutstanding = expectedContribution > 0 
      ? (outstandingAmount / expectedContribution) * 100 
      : 0;
    ...
    <div class="section">
        ...
        <div class="financial-summary">
            <div class="financial-grid">
                ...
                <div class="financial-card">
                    <div class="financial-value">${correctPercentageOutstanding.toFixed(1)}%</div>
                    <div class="financial-label">Percentage Outstanding</div>
                </div>
                <div class="financial-card">
                    <div class="financial-value">R ${expectedContribution.toLocaleString('en-ZA')}</div>
                    <div class="financial-label">Expected Contribution</div>
                </div>
            </div>
        </div>
    </div>
