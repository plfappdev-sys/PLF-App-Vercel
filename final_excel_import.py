#!/usr/bin/env python3
"""
Final Excel Import for PLF Database
Purpose: Import member data from Excel using the correct database schema
"""
...
class FinalExcelImporter:
        ...
        try:
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    # Create member data object matching the database schema
                    member_data = {
            ...
            for member in self.members_data:
                try:
                    ...
                    if existing_member.data and len(existing_member.data) > 0:
                        ...
                        update_data = member.copy()
                        del update_data['member_number']  # Don't update member_number
                        del update_data['created_at']  # Don't update created_at
                        ...
                        updated_count += 1
                        print(f"  🔄 Updated member: {member_number}")
                    else:
                        # Insert new member
                        ...
                        if response.data and len(response.data) > 0:
                            ...
                            imported_count += 1
                            print(f"  ✅ Imported member: {member_number}")
                        else:
        ...
        try:
            ...
            for member in self.members_data:
                try:
                    ...
                    success_count += 1
                    print(f"  ✅ Balance created for member {member['member_number']}: R{closing_balance:,.2f}")
                    
                except Exception as e:
                    print(f"  ❌ Error creating balance for member {member.get('member_number', 'unknown')}: {e}")
                    error_count += 1
        ...
        try:
            ...
            if sample_members.data:
                for member in sample_members.data:
                    ...
                    print(f"  - {name} (Member {member['member_number']}) - Balance: R{balance:,.2f}")
    ...
    def run_import(self):
        ...
        for member in self.members_data:
            financial_info = json.loads(member['financial_info'])
            total_balance += financial_info.get('current_balance', 0)
...
def main():
    ...
    importer = FinalExcelImporter()
    
    # Run import
    success = importer.run_import()
    
    if success:
        print("\n✅ Final data import completed successfully!")
    else:
        print("\n❌ Final data import failed. Please check the errors above.")
